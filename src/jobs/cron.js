...
function initCronJobs(client) {
  ...
  // Inactivity check every 2 hours (VA who hasn't posted in 4+ hours during active hours)
  cron.schedule('0 10,12,14,16,18,20,22 * * *', async function() {
    try { await sendInactivityAlert(); } catch (err) { console.error('Inactivity alert failed', err.message); }
  }, { timezone: config.timezone });

  // Performance drop check at 18h (compare today vs 7-day avg)
  cron.schedule('0 18 * * *', async function() {
    try { await sendPerformanceDropAlert(); } catch (err) { console.error('Perf drop alert failed', err.message); }
  }, { timezone: config.timezone });

  console.log('Cron jobs initialized (with alerts + classement + coaching + streaks)');
}
...
async function sendDailySummary() {
  ...
  try {
    ...
    // Update streaks for all VAs
    var streakMessages = [];
    for (var member3 of vaMembers.values()) {
      var streakResult = await db.updateStreak(member3.id, member3.displayName, today);
...
async function sendInactivityAlert() {
  ...
  try {
    ...
    var inactiveVAs = await db.getInactiveVAs(4);
    if (inactiveVAs.length === 0) return;

    // Only alert during working hours (check if any VA with role hasn't posted)
    var guild = await discordClient.guilds.fetch(config.discord.guildId);
    await guild.members.fetch();
    var vaMembers = guild.members.cache.filter(function(m) {
      return m.roles.cache.has(config.discord.vaRoleId) && !m.user.bot;
    ...
    await alertsChannel.send({
      content: '**⚠️ VA inactifs depuis 4h+**\n\n' + lines.join('\n') + '\n\nPensez a poster regulierement !'
    });

    console.log('Inactivity alert sent: ' + relevantInactive.length + ' VAs');
  } catch (err) {
...
async function sendPerformanceDropAlert() {
  ...
  try {
    ...
    var drops = await db.getPerformanceDrops();
    if (drops.length === 0) return;

    var lines = drops.map(function(d) {
      return '📉 **' + d.va_name + '** — ' + fmt(Number(d.today_views)) + ' vues aujourd\'hui vs ' + fmt(Number(d.avg_views)) + ' en moyenne (' + d.pct_of_avg + '% de la moyenne)';
    });

    await alertsChannel.send({
      content: '**📉 Chute de performance detectee**\n\nCes VA sont en dessous de 50% de leur moyenne sur 7 jours :\n\n' + lines.join('\n')
    });

    console.log('Performance drop alert sent: ' + drops.length + ' VAs');
  } catch (err) {
    console.error('Performance drop alert failed', err.message);
  }
}
